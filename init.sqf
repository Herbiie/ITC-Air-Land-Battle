...
if (isServer) then {
	missionNameSpace setVariable ["allPatrols",[],true];
	[] call H_fnc_equipment;
	missionNameSpace setVariable ["currentCommander",[],true];
	missionNameSpace setVariable ["subCommanders",[],true];
...
if ((isServer && ("LoadPrevious" call BIS_fnc_getParamValue == 0)) || (isServer && ("LoadPrevious" call BIS_fnc_getParamValue == 1) && (isNil "H_saved"))) then {
	missionNameSpace setVariable ["H_alb_fobs",[],true];	
	H_alb_locations = [];
	missionNameSpace setVariable ["H_alb_deploypoints",150,true];
	missionNameSpace setVariable ["H_alb_westVehicles",[],true];
	missionNameSpace setVariable ["H_alb_playerIDs",[],true];
	missionNameSpace setVariable ["H_alb_players",[],true];
	[] execVM "core\scripts\findTowns.sqf";
	[] execVM "core\scripts\handlepoints.sqf";
	missionNameSpace setVariable ["H_alb_gearTier",0,true];
