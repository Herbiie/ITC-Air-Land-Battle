if ((missionNameSpace getVariable "H_alb_deploypoints") >= 500) then {
	openMap [true, false];
	addMissionEventHandler ["MapSingleClick", {
		...
		if (_fobpos isEqualTo [0,0,0]) then { 
			...
			missionNameSpace setVariable ["H_alb_deploypoints",(missionNameSpace getVariable "H_alb_deploypoints")-500,true];
			["Notification",["FOB Deployed",format ["FOB deployed at %1.<br/>%2 Deployment Points Remaining.", mapGridPosition _fobpos, (missionNameSpace getVariable "H_alb_deploypoints")]]] remoteExec ["BIS_fnc_showNotification",0];
			openMap [false, false];
		};
	}];
} else {
